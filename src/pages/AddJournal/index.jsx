...
const AddJournal = () => {
	...
	const addEntry = async () => {
		if (date && title && content) {
			try {
				...
				const response = await axios.post('http://127.0.0.1:5000/entries/add', data, {
					headers: {
	...
	return (
		<>
			<form role="form" onSubmit={journalSubmitHandler} className="add-journal-container">
				...
				<div className="date-add-journal-container">
					...
					<input className="add-journal-text" onChange={titleHandler} type="text" value={title} placeholder="How do you feel today?" />
				</div>
				<div className="date-add-journal-container">
					<label className="content-label">Content</label>
					<div className="image-container-add-journal">
						<textarea onChange={contentHandler} type="text" value={content} className="text-area" placeholder="What's on your mind? Write it down." />
